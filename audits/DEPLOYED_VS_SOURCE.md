...
For reviewers and audit firms. Last updated 2026-07-20. Canonical web version: [kerne.fi/security/deployed-vs-source](https://kerne.fi/security/deployed-vs-source).

On any young protocol the repository moves faster than the chain. This document is the canonical table of every place where Kerne's deployed bytecode behaves differently from the current source, published ahead of the external audit, so a reviewer reading the code finds context here rather than surprises there.
...
2. **Fixed and deployed in the 2026-06-16 ceremony.** KerneVault v2 (`0x8ccc56B5...292B`) and KUSDPSM v3 (`0x07eBb486...5993`) were deployed and took over the mint path at that time. The vault and PSM findings that ceremony targeted are fixed on chain as of that deployment, and the price-oracle router the vault reads (`priceOracle`) is deployed and wired. The mint path has since moved again: on 2026-07-10 the PSM was redeployed to `0xaBDE1138...9803` and `MINTER_ROLE` was revoked on KUSDPSM v3 `0x07eBb486...5993`, which is now retired from minting and retained redeem-only (its USDC reserve still backs the kUSD minted through it until reserves migrate). Today kUSD `MINTER_ROLE` is held by exactly two contracts, KerneVault v2 `0x8ccc56B5...292B` and the live KUSDPSM `0xaBDE1138...9803` (confirm on chain, and see the verification table in the [root README](../README.md)). Any residual per-finding on-chain status is tracked in the findings tracker and the `gaps` array of [kerne.fi/api/risk-status](https://kerne.fi/api/risk-status).
