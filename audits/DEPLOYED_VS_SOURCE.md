...
For reviewers and audit firms. Last updated 2026-06-12. Canonical web version: [kerne.fi/security/deployed-vs-source](https://kerne.fi/security/deployed-vs-source).
...
| skUSD | `0xdEd74F7E06efc76455C07418b8b74Cc2bc009DB4` (deployed 2026-05-11) | `distributeYield` credits each distribution to the share price immediately, with no streaming or vesting window; a depositor bracketing a distribution captures a share proportional to transient capital. Self-found and classified High severity 2026-05-28. | The upgraded staking vault (skUSD v2) streams distributions over a vesting window, removing the bracket incentive. | Bounded by distribution size: only the amount distributed in a single event is capturable, never principal. Distributions at current scale are small, and the protocol controls their timing and size. | Distributions stay small and frequent so the bracketable amount per event stays negligible. Staking of institutional-scale capital is explicitly gated on the skUSD v2 deployment plus the external audit. The v2 redeploy decision is pending and tracked. |
