...
contract KUSDPSM is AccessControl, ReentrancyGuard, Pausable, IERC3156FlashLender {
    ...
    /// @dev KRN-26-PSM-REDEEM-NO-BURN: the redeem-path self-burn of returned kUSD failed. The PSM
    ///      holds the kUSD it just pulled in, so ERC20Burnable.burn cannot fail on a live kUSD; a
    ///      revert here means the token unexpectedly lacks a working burn(uint256), and failing
    ///      closed is safer than silently re-accreting dead inventory and inflating supply.
    error KusdBurnFailed();
    ...
    function _swapKUSDForStableInner(
        ...
        // SECURITY (KRN-26-PSM-REDEEM-NO-BURN, 2026-07-06,
        // docs/security/PSM_REDEEM_NO_BURN_2026-07-06.md): burn the redeemed kUSD when the PSM is
        // in mint-fresh mode, mirroring the swap-in branch (which MINTS new kUSD when mintingEnabled
        // and DISPENSES held inventory otherwise). Without this, a mint->redeem round-trip left the
        // returned kUSD parked in the PSM as dead inventory while the next mint minted anew, so raw
        // kUSD.totalSupply() inflated without bound past real backing — a caller could repeat the
        // loop (paying only fees) and blow up the reported supply that the kerne.fi/verify tool
        // reads, turning our own transparency rail against kUSD. Self-burn (ERC20Burnable.burn)
        // destroys the PSM's OWN just-received balance and needs no role. When mintingEnabled is
        // false the PSM is a fixed-inventory swap desk: the returned kUSD is KEPT so the next
        // swap-in can dispense it, and supply is untouched on both legs. The full kUSD `amount` is
        // burned (the fee is retained as stable, never as kUSD), so a complete round-trip nets to
        // fees only.
        if (mintingEnabled) {
            (bool burned,) = address(kUSD).call(abi.encodeWithSignature("burn(uint256)", amount));
            if (!(burned)) revert KusdBurnFailed();
        }
