...
- **Four** describe real bug classes that were already identified and fixed in Kerne's own May to June 2026 internal audit cycle. The live, source-verified bytecode already carries the fix; the submitted proofs were written against a pre-fix snapshot and do not reproduce on deployed code.
  ...
  **Read "already fixed" as a statement about the code, never about who found it.** Corrected 2026-08-14. Every one of those four was **also independently reported by this team**, in writing, in June 2026, and finding 6 (withdrawal-limit enforcement) is the one where that distinction has actually been blurred elsewhere: the withdrawal-breaker class has been described on Kerne's own surfaces as internally found, without the qualifier that an outside team reached it too and told us. That framing is wrong and it is withdrawn here. An external report that lands after an internal fix is still an external report, and a protocol that only credits the reports it did not already know about is grading its own homework. The two hardening notes in the "What we did with it" list below, including the claim-time re-check on the withdrawal breaker, came out of this review and not out of an internal pass.
...
- **One** is a genuine, currently-inert, pre-launch accounting item. It independently confirms a finding Kerne had already documented in its May 8 internal audit: in the vault's outflow path, on-chain tracked assets are decremented before the matching off-chain or bridged bucket is credited, so a deposit landing inside that reconciliation window could over-mint. It is inert on the live deployment because the vault is empty: any such operation reverts on checked-math underflow, and with `totalSupply()` at 0 there are no holders to dilute. **Correction, 2026-07-28:** an earlier revision of this paragraph added "and the deposit interface is not open" as a second reason. That was misleading. Only the app interface is closed; the contract itself accepts deposits, because the on-chain deposit gate was never deployed and `maxDeposit` returns 2^256-1. The inertness above rests on the empty-vault argument alone, and the on-chain closure is proposed and awaiting a second Safe signature. See [`DEPLOYED_VS_SOURCE.md`](DEPLOYED_VS_SOURCE.md). It is now fixed in source (atomic bucket crediting, plus a real on-chain deposit gate to replace interface-only gating), staged for the next vault redeployment, and treated as a pre-launch blocker. The fix is not yet on chain; its status will appear in [`DEPLOYED_VS_SOURCE.md`](DEPLOYED_VS_SOURCE.md) and the findings tracker when it deploys.
...
| 6 | Withdrawal-limit enforcement | Externally reported by this team, June 2026. Already fixed on Kerne's side before the report landed: the limit is enforced on the live exit path. Both halves of that sentence are true and the first one belongs in it | No |
