...
contract KerneVault is ERC4626, AccessControl, ReentrancyGuard, Pausable {
    ...
    /// @notice Role for the automated keeper network (KerneKeeper) to poke the CR circuit breaker.
    /// @dev Deliberately narrower than PAUSER_ROLE: a KEEPER_ROLE holder can ONLY run the
    ///      state-determined `updateCircuitBreaker()` (which trips/clears the breaker strictly
    ///      according to the live on-chain collateral ratio) — it cannot arbitrarily pause a healthy
    ///      vault, move funds, or touch any other privileged path. PAUSER_ROLE stays with the
    ///      multisig for discretionary emergency pauses. Granted to the KerneKeeper CONTRACT address.
    ...
    /// @notice Highest vault-share balance each account has ever held (its position high-water
    ///         mark), used ONLY to detect an economic exit for esKERNE forfeiture.
    /// @dev SECURITY (KRN-26-ESK-FORFEIT-DUST-BYPASS, 2026-07-06, Yaroslav F3,
    ///      docs/security/KERNEVAULT_FORFEIT_DUST_BYPASS_2026-07-06.md): the pre-fix forfeiture
    ///      trigger in `requestWithdrawal` fired only when the caller's POST-transfer balance fell
    ///      below a FLAT dust floor (`10 ** _decimalsOffset()`, one asset-wei), independent of
    ///      position size. A large holder could therefore withdraw all-but-a-few-wei, sit just above
    ///      the floor, and keep 100% of their unvested esKERNE. This tracks the account's peak so the
    ///      exit test compares the residual to a FRACTION of that peak instead. Maintained in
    ///      `_update` on EVERY inbound balance change (deposit-mint AND inbound transfer), so a
    ///      position funded by transfer raises the peak just like a deposit and cannot present a
    ///      tiny peak to slip under the dust floor (closes the two-address / transfer-funded farm).
    ///      It only ratchets UP in `_update`, so a third party cannot lower it, and it is grief-safe
    ///      (a gifted inbound transfer raises balance and peak together). It is ratcheted DOWN to the
    ///      residual whenever forfeiture fires so a later re-deposit starts a fresh high-water.
    ///      NOTE: this governs the WITHDRAWAL path only; exiting by transferring shares to another
    ///      wallet and never withdrawing is a separate, known, out-of-scope vector (see the doc).
    mapping(address => uint256) public forfeitureHighWaterShares;

    /// @notice Fraction (bps) of an account's peak position it must RETAIN to avoid an esKERNE
    ///         forfeiture on withdrawal. Below this, a withdrawal is treated as an economic exit.
    /// @dev KRN-26-ESK-FORFEIT-DUST-BYPASS. 100 bps (1%) is the security-minimal line that closes
    ///      the size-independent dust bypass while preserving legitimate partial withdrawals
    ///      (anything short of pulling >99% of the peak keeps esKERNE). Salami-sliced exits cannot
    ///      escape either: because the peak is fixed at the deposit high-water, the residual must
    ///      stay at/above 1% of peak across ALL withdrawals to retain esKERNE. A stricter retention
    ///      policy is a protocol-economics tuning knob, out of scope for this security fix.
    uint256 public constant FORFEIT_EXIT_RETAIN_BPS = 100; // 1% of peak
    ...
    /// @notice Master on-chain switch for public deposits (KRN-26-VAULT-DEPOSITS-UI-ONLY-GATE).
    /// @dev SECURITY: pre-launch, deposits are "closed" only because the app UI points at the
    ///      retired v1 vault — there is no on-chain control, so anyone could deposit directly into
    ///      v2. This flag IS that control: when false, maxDeposit()/maxMint() return 0 and
    ///      _deposit() reverts DepositsDisabled, while EVERY other operation (sweep, L1 bridge,
    ///      withdrawals, config) stays available — unlike pause(), which freezes the whole vault.
    ///      Defaults to TRUE so existing flows/tests are byte-for-byte unchanged; the launch
    ///      ceremony calls setDepositsEnabled(false) to hold the vault shut until go-live, exactly
    ///      like deploying paused. CLONE-INIT: re-applied in _initialize() because EIP-1167 clones
    ///      never run inline initializers (a clone would otherwise be born deposits-CLOSED).
    ...
    /// @notice Minimum gas that must remain immediately before the best-effort esKERNE
    ///         forfeit() sub-call in requestWithdrawal (KERNE1-5, Hexens Jul-2026).
    /// @dev Under EIP-150 only 63/64 of the remaining gas is forwarded to an external call,
    ///      so an exiting caller could set a tx gas limit that starves forfeit() into an
    ///      out-of-gas revert while the parent frame retains its ~1/64 and still persists the
    ///      withdrawal, keeping 100% of their unvested esKERNE. Requiring this floor before the
    ///      call guarantees the forwarded 63/64 covers forfeit()'s cost, so the attacker's only
    ///      options are "forfeiture runs" or "the whole withdrawal reverts", never
    ///      "withdrawal without forfeiture". 200k leaves ample margin over forfeit()'s real cost.
    uint256 public constant MIN_FORFEIT_GAS = 200000;
    ...
    /// @notice Emitted when the on-chain public-deposits gate is toggled (KRN-26-VAULT-DEPOSITS-UI-ONLY-GATE).
    ...
    /// @dev KERNE1-2 (Hexens Jul-2026): thrown when a strategist tries to write a bootstrapped
    ///      asset bucket (offChainAssets / hedgingReserve / l1Assets) whose current value is 0.
    ///      Once bootstrapped, the rate-limit baseline collapses to 1 wei and any non-zero write
    ///      always exceeds (baseline * maxOffChainChangeRateBps)/10000 == 0, so the bucket would be
    ///      permanently stuck at 0. This dedicated error signals the intended recovery path:
    ///      DEFAULT_ADMIN_ROLE must call resetBucketBootstrap() before the bucket can be re-seeded.
    error BucketBootstrapResetRequired();

    /// @dev KERNE1-5 (Hexens Jul-2026): thrown when insufficient gas remains to safely forward the
    ///      best-effort esKERNE forfeit() sub-call, preventing a gas-starvation forfeiture bypass.
    error InsufficientGasForForfeit();
    ...
    /// @dev SECURITY (KRN-26-VAULT-DEPOSITS-UI-ONLY-GATE): thrown by _deposit() when the
    ///      admin-controlled on-chain deposits gate is closed (depositsEnabled == false).
    ...
    function _initialize(
        ...
        // KRN-26-VAULT-DEPOSITS-UI-ONLY-GATE: clones never run the inline `= true` initializer,
        // so re-apply it here — a clone must be born deposits-ENABLED to match a constructor
        // deployment (the launch ceremony then closes it explicitly until go-live).
    ...
    /// @dev SECURITY (KRN-26-ESK-FORFEIT-DUST-BYPASS): maintain each account's forfeiture high-water
    ///      (peak share balance) on EVERY inbound balance change, not only own-deposits. `_mint`
    ///      (deposit), inbound `_transfer`, and every other share credit route through this ERC-20
    ///      hook, so a position funded by an inbound transfer raises the receiver's peak exactly like
    ///      a deposit would. This closes the two-address / transfer-funded farm: the esKERNE-holding
    ///      address can no longer present a tiny deposit-only peak while economically exiting a large
    ///      transfer-funded position (the withdrawal-side exit test in `requestWithdrawal` measures
    ///      the residual against this peak). It is monotone-up here (only ratcheted DOWN on a
    ///      successful forfeit in `requestWithdrawal`), so it cannot be lowered by a third party, and
    ///      it is grief-safe: a gifted inbound transfer raises the receiver's balance AND peak by the
    ///      same amount, so the gift stays as residual and can never push a victim below their exit
    ///      floor. address(0) (mint source / burn destination) is skipped.
    function _update(
        address from,
        address to,
        uint256 value
        ...
        if (to != address(0)) {
            ...
            if (balAfter > forfeitureHighWaterShares[to]) {
                forfeitureHighWaterShares[to] = balAfter;
            }
        }
    }
     ...
     *      `totalPrimeAllocated` (principal out at prime accounts) is added in EVERY branch — it is
     *      on-chain-tracked vault property the off-chain node does not report (KRN-26-VAULT-PRIME-NAV-GAP).
    ...
    function totalAssets() public view virtual override returns (uint256) {
        ...
        if (node != address(0)) {
            ...
            // KERNE1-7 (Hexens Jul-2026): on a verification-node failure (revert or non-32-byte
            // return) fall back to the SAME four-bucket no-node formula rather than dropping the
            // strategist-maintained off-chain buckets. The previous fallback returned only
            // _trackedOnChainAssets + totalPrimeAllocated, silently excluding offChainAssets +
            // l1Assets + hedgingReserve for the whole node-outage window. Because totalAssets()
            // drives share pricing, solvency checks, and the deposit cap, that under-report let
            // depositors mint at a depressed price and later redeem into the recovered NAV at the
            // expense of existing holders (and an attacker who could force the node to revert could
            // manufacture the window on demand). Falling back to the last strategist-attested
            // buckets keeps NAV continuous across a transient node outage.
    ...
    // KERNE1-6 (Hexens Jul-2026): the previously-defined internal _checkSolvency(bool) wrapper was
    // never called anywhere in the codebase (all solvency paths call _updateSolvency directly), so
    // it was removed as dead code. checkAndPause() and the strict revert paths call _updateSolvency.
    ...
    function updateOffChainAssets(
        ...
        if (_offChainAssetsBootstrapped && maxOffChainChangeRateBps > 0) {
            // KERNE1-2 (Hexens Jul-2026): once bootstrapped, a bucket sitting at 0 cannot be
            // rate-limited back to a non-zero value. The prior code used a 1-wei baseline, but
            // (1 * maxOffChainChangeRateBps) / 10000 == 0 for every allowed rate (max 50%), so any
            // non-zero write exceeds maxChange and reverts, leaving the bucket permanently stuck at
            // 0. Surface the intended recovery path with a dedicated error instead of the
            // misleading OffChainChangeExceedsMaxRate: DEFAULT_ADMIN_ROLE must call
            // resetBucketBootstrap() to deliberately re-seed the bucket (the same multisig-gated
            // override that guards the original zero-then-restore rate-limit bypass).
            if (oldAmount == 0) revert BucketBootstrapResetRequired();
    ...
    function updateHedgingReserve(
        ...
        if (_hedgingReserveBootstrapped && maxOffChainChangeRateBps > 0) {
            // KERNE1-2 (Hexens Jul-2026): see updateOffChainAssets. A bootstrapped bucket at 0
            // cannot be rate-limited back to non-zero (baseline collapses to a value that yields
            // maxChange == 0), so require a deliberate admin resetBucketBootstrap() to re-seed it.
            if (oldAmount == 0) revert BucketBootstrapResetRequired();
            uint256 maxChange = (oldAmount * maxOffChainChangeRateBps) / 10000;
            uint256 change = amount > oldAmount ? amount - oldAmount : oldAmount - amount;
            if (change > maxChange) revert OffChainChangeExceedsMaxRate();
    ...
    function sweepToExchange(
        ...
        // SECURITY FIX (KRN-26-VAULT-RECLASS-NAV-GAP, 2026-06-28,
        // docs/security/KERNEVAULT_RECLASS_NAV_GAP_2026-06-28.md; Zellic-affiliated review
        // finding #04 "accounting-gap-dilutes-holders"): a sweep MOVES on-chain principal to the
        // CEX hedging venue, it does not destroy value. Atomically reclassify the swept amount
        // from the on-chain ledger into the off-chain bucket so totalAssets()
        // (= _trackedOnChainAssets + offChainAssets + l1Assets + hedgingReserve) is INVARIANT
        // across the handoff. Pre-fix the off-chain bucket was raised only LATER by a separate
        // STRATEGIST updateOffChainAssets() call, so between the sweep and that report
        // totalAssets() was understated by `amount`; a deposit landing in that window minted
        // shares at a depressed price and diluted existing holders once the moved principal was
        // re-reported. updateOffChainAssets() uses absolute SET semantics, so it now only
        // TRUES-UP this bucket to the mark-to-market value and never re-counts the principal
        // credited here (no double-count). This admin-authorized, value-preserving move is
        // itself exempt from the strategist rate-limit/cooldown. It deliberately leaves the
        // bootstrap / rate-limit state UNTOUCHED so updateOffChainAssets()'s report semantics are
        // byte-for-byte unchanged (bucket bootstrap stays owned solely by that setter). This adds
        // no new drain vector: the first strategist mark after a sweep was already unconstrained
        // pre-fix — offChainAssets was simply 0 at that point instead of holding `amount`.
        _trackedOnChainAssets -= amount;
        offChainAssets += amount;
        emit OffChainAssetsUpdated(offChainAssets - amount, offChainAssets, block.timestamp);
    ...
    /// @notice Opens or closes public deposits at the contract level (KRN-26-VAULT-DEPOSITS-UI-ONLY-GATE).
    /// @dev Only callable by DEFAULT_ADMIN_ROLE (the Safe). When false, maxDeposit()/maxMint()
    ///      return 0 and deposit()/mint() revert DepositsDisabled, while withdrawals, sweeps, L1
    ///      bridging and config remain available. Used to hold the vault shut pre-launch and to
    ///      re-close it without the broad freeze of pause().
    /// @param enabled True to allow deposits, false to close them.
    ...
    function captureFounderWealth(
        ...
        // KERNE1-8 (Hexens Jul-2026): rewardReserveAmount + founderRevenueAmount always equals
        // totalFee (founderRevenueAmount is defined as totalFee - rewardReserveAmount after the
        // clamp above), so use totalFee directly instead of the redundant `extracted` variable.
        uint256 maxExtractable = (_trackedOnChainAssets * maxFounderCaptureBps) / 10000;
        if (totalFee > maxExtractable) revert FounderCaptureExceedsLimit(totalFee, maxExtractable);
        ...
        // KERNE1-9 (Hexens Jul-2026): captureFounderWealth reduces _trackedOnChainAssets (and thus
        // totalAssets()) by up to maxFounderCaptureBps, so it can lower the collateral ratio exactly
        // like the three strategist bucket updates that already run this check. Evaluate the CR
        // circuit breaker here too so a capture that pushes the vault below CRITICAL_CR_THRESHOLD
        // arms the Red Halt immediately instead of leaving users transacting on a mispriced share.
        _checkCRCircuitBreaker();
    }
    ...
    function requestL1Deposit(
        ...
        // SECURITY FIX (KRN-26-VAULT-RECLASS-NAV-GAP, 2026-06-28): same defect and remedy as
        // sweepToExchange (see its NatSpec). Bridging to the Hyperliquid L1 moves principal, it
        // does not destroy it — atomically reclassify the bridged amount into the l1Assets bucket
        // so totalAssets() is invariant across the handoff and a deposit in the pre-reconcile
        // window cannot mint at a depressed price. updateL1Assets() (absolute SET) then trues-up
        // l1Assets to the L1 mark without re-counting this principal. Leaves the bootstrap /
        // rate-limit state untouched (owned solely by updateL1Assets), preserving its report
        // semantics and adding no new drain vector.
        _trackedOnChainAssets -= amount;
        l1Assets += amount;
        emit L1AssetsUpdated(l1Assets - amount, l1Assets, block.timestamp);
    ...
    function requestWithdrawal(
        ...
        //
        // Exit detection (KRN-26-ESK-FORFEIT-DUST-BYPASS, 2026-07-06, Yaroslav F3,
        // docs/security/KERNEVAULT_FORFEIT_DUST_BYPASS_2026-07-06.md): forfeiture fires when the
        // caller's POST-transfer residual falls below the GREATER of two floors, not a single flat
        // dust threshold:
        //   - the absolute dust floor `10 ** _decimalsOffset()` shares (< one asset-wei), which
        //     covers a genuine "withdraw all" whose `previewWithdraw` ceil leaves sub-wei virtual-
        //     offset dust — the original KRN-26-ESK-FORFEIT-UNWIRED full-exit case; AND
        //   - `FORFEIT_EXIT_RETAIN_BPS` (1%) of the account's PEAK share balance
        //     (`forfeitureHighWaterShares`), which covers a LARGE position dropped to token dust.
        // The pre-fix code used ONLY the absolute floor. Because that floor is independent of
        // position size, a large holder could withdraw all-but-a-few-wei, keep a residual just above
        // one asset-wei, and retain 100% of their unvested esKERNE (the F3 bypass). Keying the second
        // floor to the holder's own peak (tracked on every inbound balance change in `_update`) makes
        // "economic exit" scale with the position, so on the WITHDRAWAL path an all-but-dust exit, a
        // salami-sliced exit, and a transfer-funded position all forfeit, while genuine partial
        // withdrawals (residual >= 1% of peak) keep their esKERNE. On a successful forfeit the peak is
        // re-anchored to the residual so a later re-deposit starts a fresh high-water.
        //
        // SCOPE: this closes the withdrawal-path bypass Yaroslav reported. It does NOT cover exiting
        // by transferring shares to another wallet and never calling requestWithdrawal (forfeiture is
        // only driven from here) — a separate, pre-existing, known-latent vector that needs a
        // transfer hook or non-transferable shares, deliberately out of scope for this fix (see doc).
        //
        ...
        if (esc != address(0)) {
            ...
            if (residualShares < exitFloor) {
                // KERNE1-5 (Hexens Jul-2026): guarantee enough gas remains that the 63/64 forwarded
                // under EIP-150 covers forfeit()'s full cost. Without this floor an exiting caller
                // could pick a tx gas limit that starves the forfeit() sub-call into an out-of-gas
                // revert (swallowed by the catch below) while the parent frame keeps its ~1/64 and
                // still persists the withdrawal, keeping 100% of their unvested esKERNE. With the
                // floor the only outcomes are "forfeiture runs" or "the whole withdrawal reverts",
                // never "withdrawal succeeds but forfeiture is skipped".
    ...
    ///      refined by KERNE1-11, Hexens 2026-07): the payout is the LESSER of the claim-time share
    ///      value and the request-time asset snapshot, `min(convertToAssets(req.shares), req.assets)`.
    ///      This unifies two directional protections:
    ///        - DOWNSIDE taken at CLAIM time. The original code paid `req.assets` literally, so a
    ///          claimer collected the full request-time value even after a loss during cooldown, a
    ///          free put for anyone who queued before an expected drawdown that socialised the
    ///          residual loss across non-queueing holders. On a drawdown
    ///          `convertToAssets(req.shares) < req.assets`, so the smaller claim-time value is paid
    ///          and no over-transfer occurs (the 2026-05-23 fix).
    ///        - UPSIDE capped at REQUEST time (KERNE1-11). A withdrawal request is an irreversible
    ///          exit commitment, so the claimant forfeits yield accrued after the request rather
    ///          than being able to wait out the cooldown and selectively claim only when NAV has
    ///          risen, or front-run a pending updateL1Assets loss. Any post-request upside
    ///          (`convertToAssets(req.shares) > req.assets`) stays in the vault and accrues to the
    ///          remaining holders.
    ///      `req.assets` is therefore load-bearing as the UPPER bound; the buffer check,
    ///      tracked-assets decrement, transfer amount, and emitted event all derive from the
    ///      resulting `assetsOut` below.
    function claimWithdrawal(
        ...
        uint256 assetsOut = Math.min(convertToAssets(req.shares), req.assets);
        ...
        // raw token balance. `assetsOut` is at most `convertToAssets(req.shares)` (capped at
        // `req.assets` per KERNE1-11), scaled against the full four-bucket `totalAssets()`
        // (on-chain + offChain + l1 + hedgingReserve).
    ...
    /// @notice Aggregate of vault principal currently allocated out to ALL prime accounts.
    /// @dev SECURITY FIX (KRN-26-VAULT-PRIME-NAV-GAP, 2026-06-28,
    ///      docs/security/KERNEVAULT_PRIME_NAV_GAP_2026-06-28.md): the sibling of finding #04
    ///      (KRN-26-VAULT-RECLASS-NAV-GAP) on the `transferToPrime`/`returnFromPrime` path.
    ///      `transferToPrime` moved principal OUT of `_trackedOnChainAssets` while crediting only
    ///      `primeAccounts[prime].allocatedAssets`, which is NOT a term of `totalAssets()`. During
    ///      the allocation window `totalAssets()` was understated by the allocated amount, so a
    ///      deposit landing in that window minted shares at a depressed price and diluted holders
    ///      once the principal was re-counted. This aggregate is the matching off-venue bucket for
    ///      the prime path: it is incremented in `transferToPrime` and added to `totalAssets()` in
    ///      BOTH the verification-node branch and the four-bucket fallback (a prime allocation is a
    ///      purely on-chain move the off-chain node cannot see), so the transfer is a value-
    ///      preserving reclassification rather than a transient disappearance. Unlike sweep/L1,
    ///      this path is COUPLED to `returnFromPrime` (which auto-credits `_trackedOnChainAssets`
    ///      by balance-delta on return); to avoid double-counting the yield, this aggregate is
    ///      decremented in `returnFromPrime` by the PRINCIPAL portion only (min(allocated, amount)),
    ///      so principal returns are NAV-neutral and only realized yield (received - principal)
    ///      lands once. Invariant: `totalPrimeAllocated == Σ primeAccounts[*].allocatedAssets`.
    uint256 public totalPrimeAllocated;
    ...
    ///      `_trackedOnChainAssets` and added to BOTH the prime account's `allocatedAssets` tracking
    ///      and the `totalPrimeAllocated` aggregate. Because `totalPrimeAllocated` is a term of
    ///      `totalAssets()` (KRN-26-VAULT-PRIME-NAV-GAP), this move is a value-preserving
    ///      reclassification: NAV is invariant across the transfer, so a deposit landing in the
    ///      allocation window can no longer mint at a depressed price and dilute holders.
    ...
    function returnFromPrime(
        ...
        uint256 allocated = primeAccounts[prime].allocatedAssets;
        // KRN-26-VAULT-PRIME-NAV-GAP: the PRINCIPAL portion of this return — min(allocated, amount) —
        // is the part already counted in totalAssets() via totalPrimeAllocated. Decrement BOTH ledgers
        // by exactly that portion so returning principal is NAV-neutral, while only the realized yield
        // (received - principal) lands once via the _trackedOnChainAssets balance-delta credit below.
        // No double-count: the yield was never in totalPrimeAllocated, so it is recognised exactly once.
        // Safe: principalPortion <= allocated <= totalPrimeAllocated (the aggregate is the sum of all
        // per-account allocations), so the aggregate decrement cannot underflow.
        uint256 principalPortion = allocated >= amount ? amount : allocated;
        primeAccounts[prime].allocatedAssets = allocated - principalPortion;
        totalPrimeAllocated -= principalPortion;
    ...
    function maxDeposit(
        ...
        if (paused()) return 0;
        // KRN-26-VAULT-DEPOSITS-UI-ONLY-GATE: real on-chain stop for the closed-deposits window.
    ...
    function _deposit(
        ...
        // KRN-26-VAULT-DEPOSITS-UI-ONLY-GATE: hard on-chain stop, mirrored by maxDeposit() == 0.
        ...
        // KERNE1-3 (Hexens Jul-2026): enforce the maxTotalAssets cap against the NET amount that will
        // actually be added to _trackedOnChainAssets. That net amount is `assets - fee` ONLY when the
        // fee leaves the vault to a recipient; when the fee is absorbed (BUG-06 path below, neither
        // treasury nor founder set) the full `assets` is tracked. We therefore resolve the EFFECTIVE
        // fee first, then use that single figure in both the cap check and the tracked-assets add, so
        // the cap can neither over-estimate the post-deposit total (the original KERNE1-3 bug, which
        // reverted valid deposits) nor under-estimate it (an absorb-branch overshoot). `fee` never
        // exceeds `assets` (depositFeeBps is capped well below 100%), so `assets - fee` cannot
        // underflow.
        uint256 fee = (assets * depositFeeBps) / 10000;
        // BUG-06 FIX: the original code silently kept a non-zero fee inside the vault while excluding
        // it from _trackedOnChainAssets, creating a permanent accounting gap that inflated the share
        // price. Route the fee to treasury, else founder, else (neither set, e.g. a white-label clone
        // where founder may be address(0)) absorb it into tracked assets by zeroing the effective fee.
        address feeRecipient = fee > 0 ? (treasury != address(0) ? treasury : founder) : address(0);
        if (fee > 0 && feeRecipient == address(0)) {
            // No recipient configured: the fee stays in the vault and counts toward tracked assets.
            fee = 0;
        }
        if (maxTotalAssets > 0 && totalAssets() + assets - fee > maxTotalAssets) revert DepositCapExceeded();
        SafeERC20.safeTransferFrom(IERC20(asset()), caller, address(this), assets);
        if (fee > 0) {
            // The fee physically leaves the vault, so it is NOT added to _trackedOnChainAssets below.
            SafeERC20.safeTransfer(IERC20(asset()), feeRecipient, fee);
        }
        _trackedOnChainAssets += assets - fee;
        _mint(receiver, shares);
        // KRN-26-ESK-FORFEIT-DUST-BYPASS: the receiver's forfeiture high-water is updated by the
        // `_update` hook that `_mint` routes through (see `_update`), so it is NOT tracked here —
        // that also captures positions funded by inbound share transfer, not only own-deposits.
    ...
    /// @notice Runs the CR circuit-breaker check on demand, tripping (Red Halt) or clearing it to
    ///         match the live collateral ratio. This is the automated keeper's entrypoint.
    /// @dev Only callable by KEEPER_ROLE (the KerneKeeper contract / monitoring bot). The vault's CR
    ///      breaker is otherwise only evaluated as a side effect of strategist bucket updates
    ///      (updateOffChainAssets / updateHedgingReserve / updateL1Assets), so without this function
    ///      KerneKeeper's actionType==2 upkeep had no on-chain entrypoint and always reverted.
    ///
    ///      Safe to expose under a narrow role because the outcome is fully determined by on-chain
    ///      state, not the caller: it can pause ONLY when getSolvencyRatio() is genuinely below
    ///      CRITICAL_CR_THRESHOLD, raise a soft alert below WARNING_CR_THRESHOLD, and clear the
    ///      breaker only at/above SAFE_CR_THRESHOLD after the cooldown. A KEEPER_ROLE holder can
    ///      therefore never falsely pause a healthy vault. Calls are idempotent.
    function updateCircuitBreaker() external onlyRole(KEEPER_ROLE) {
        _checkCRCircuitBreaker();
    }
