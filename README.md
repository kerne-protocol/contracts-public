...
- **`scripts/verify_public_endpoints.sh`** — a zero-dependency-beyond-`curl`-and-`jq` script any outsider can run against the live protocol. It runs in two parts. The first asserts that every documented public endpoint matches its published contract; those endpoints are Kerne-hosted, so that part proves the surface is live and well-formed and nothing more. The second reads kUSD, skUSD and all three PSM reserves straight off Base through a public JSON-RPC endpoint Kerne does not operate, re-derives the backing invariant from chain state with no Kerne input at all, and checks Kerne's signed Proof of Reserves against the chain at the exact block the attestation names. Exit code 0 when every check passed, 1 on a failure, 3 when no public RPC could be reached and the on-chain half therefore did not run. That last code exists so a reader's own network trouble is never reported as a Kerne defect. Point it at your own node with `BASE_RPC=`.
...
- **`docs/COMMIT_SIGNING.md`** — commit provenance. Every commit on `main` after `32f4273d` is SSH-signed and `main` rejects unsigned pushes, so an edit to the address registry above carries a signature you can check yourself. That document gives both the GitHub-side check and the offline one, names the fingerprint to pin, and states plainly that the history before `32f4273d` is unsigned and was deliberately not rewritten to hide that.
- **`SECURITY.md`**, **`audits/`**: disclosure path and audit posture. The Hexens final report published July 31, 2026 and is committed here in full: [`audits/hexens-kerne-protocol-final-2026-07-31.pdf`](audits/hexens-kerne-protocol-final-2026-07-31.pdf), and Hexens publishes the same report on their own site at [hexens.io/audit-reports/kerne-protocol-july-2026](https://hexens.io/audit-reports/kerne-protocol-july-2026), with the finding table and the deployed-versus-reviewed caveat in [`audits/README.md`](audits/README.md). Auditor scoping reference: [`audits/SCOPE.md`](audits/SCOPE.md). Deployed-vs-source state disclosure (where live bytecode differs from current source, with operating rules): [`audits/DEPLOYED_VS_SOURCE.md`](audits/DEPLOYED_VS_SOURCE.md).
...
> **Integrating skUSD? `decimals()` is 24, not 18.** skUSD is an ERC-4626 share over kUSD, and kUSD reports 18 decimals, but skUSD deliberately reports **24** (a fixed 1e6 offset). Assuming the share matches its asset misprices skUSD by a factor of one million. Price it with `convertToAssets` and never with a hardcoded ratio. Worked example in [`HOW_TO_VERIFY_KERNE.md`](HOW_TO_VERIFY_KERNE.md) section 4 and in [`audits/DEPLOYED_VS_SOURCE.md`](audits/DEPLOYED_VS_SOURCE.md).
